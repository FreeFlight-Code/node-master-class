      ...
      _data.read("users", phone, function (err, userData) {
        if (err || !userData) {
          ...
          callback(400, {
			...
			// user found convert userData to userObj
			const userObj = helpers.parsedJsonToObject(userData);
			// create tokenObj
  ...
  verifyToken: function (tokenId, phone, callback){
	  _data.read('tokens', tokenId, function(err, tokenData){
		  if(!err){
			...
			if (tokenObj.phone === phone && tokenObj.expires > Date.now()) callback(true);
			else callback(false)
		} else {
			callback(false)
		}
